...
def inference_graph(char_vocab_size, word_vocab_size,
    ...
    ind    = tf.placeholder(tf.int32, name="ind")
    ...
    with tf.variable_scope('Embedding'):
        ...
        # set the start character embedding to zero
    ...
    with tf.variable_scope('Loss'):
         y = tf.placeholder(tf.int64, [batch_size, num_unroll_steps], name='targets')
    ...
    with tf.variable_scope('LSTM'):
        ...
        outputs, final_rnn_state = tf.nn.dynamic_rnn(cell, input_cnn,
                                         initial_state=initial_rnn_state, dtype=tf.float32,time_major=False)
        with tf.variable_scope('WordEmbedding') as scope:
            #outputs = tf.unstack(tf.transpose(outputs, [1,0,2]))     #step. batch,  output_size
            with tf.variable_scope("SimpleLinear"):
                matrix = tf.get_variable("Matrix", [word_vocab_size, rnn_size], dtype=tf.float32)
                bias_term = tf.get_variable("Bias", [word_vocab_size], dtype=tf.float32)
 
        
        
        ''' do softmax and compute loss'''
        print "outputs:", outputs.shape 
        print "test:", tf.concat(outputs,1).shape
        #batch_size*steps, rnn_size
        ...
        if mode=="train":
            ...
            loss = tf.nn.sampled_softmax_loss(matrix, bias_term, targets, tf.to_float(outputs), num_sampled, word_vocab_size)
            print "loss:",loss
            loss = tf.reduce_mean(loss, name='loss')
        else:
            full_softmax_w = tf.reshape(tf.concat(matrix,1),[-1,  rnn_size])  
            #[batch_size*steps, vocab_size]
            logits = tf.matmul(outputs, full_softmax_w, transpose_b = True)  + bias_term
            ind_logits =  tf.reshape(logits,[word_vocab_size,-1])
            ind_logits =  tf.gather(ind_logits, ind)
            ind_logits =  tf.reshape(ind_logits,[-1,word_vocab_size])
            _, index   =  tf.nn.top_k(ind_logits, arg_max)
            print "index:", index.shape, type(index)
            #_, index  =  tf.nn.top_k(logits, arg_max)
            targets = tf.reshape(y,[-1]) 
            loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=targets, logits=logits)
            loss = tf.reduce_mean(loss, name = 'loss')
            print "loss:",  loss
        print "mode:", mode, loss
    ...
    return adict(
        ...
        rnn_outputs=outputs,
        logits = logits,  #  dif  in train and valid 
        index = index, 
        loss = loss,
        targets = y
    )
...
def training_graph(loss, learning_rate=1.0, max_grad_norm=5.0):
    ...
    with tf.variable_scope('SGD_Training'):
        ...
        learning_rate = tf.placeholder(tf.float32,name="init_learning_rate")
